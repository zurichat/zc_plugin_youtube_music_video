...
const chatsSlice = createSlice({
  ...
  reducers: {
    setChats: (state, { payload }: PayloadAction<Chat[]>) => payload.map(sanitize),
...
export const {
  addedChat, setChats, failChat, removeChat, sentChat,
} =	chatsSlice.actions;

export const selectChats = (state: RootState) => state.chats;

export const selectChatById = (id: string) => (state: RootState) => state.chats.find((chat) => chat.id === id);

export const selectLastChat = (state: RootState) => state.chats[state.chats.length - 1];
